...
from common import extract_filename, indexer
...
def plot_h(array, xcol_index, ycol_index, interval, file0):
    ...
    dataframe1 = pd.DataFrame(
        binned,
        columns=[
            "Tafel Bin / mV decade-1",
            "R2 threshold",
            "Overpotential Width / V",
            "Tafel slope / mV Decade-1",
            "Exchange Current / A",
            "Optimal Residual",
            "Starting Fit Overpotential / V",
            "Tafel R2",
            "LSV R2",
    ...
    mt.hist2d(array[:, 3], abs(array[:, 2]), bins=70, cmap="nipy_spectral_r")
    mt.colorbar(label="Counts")
    mt.xlabel("Tafel Slope / mV decade$^-$$^1$")
    mt.ylabel("Overpotential Fit Width / V")
    ...
    mt.hist2d(array[:, 3], array[:, 7], bins=70, cmap="nipy_spectral_r")
    mt.colorbar(label="Counts")
    mt.xlabel("Tafel Slope / mV decade$^-$$^1$")
    mt.ylabel("Tafel Fit R$^2$")
    ...
    mt.hist2d(array[:, 3], array[:, 6], bins=70, cmap="nipy_spectral_r")
    mt.colorbar(label="Counts")
    mt.xlabel("Tafel Slope / mV decade$^-$$^1$")
    mt.ylabel("Starting Fit Overpotential / V")
    ...
    mt.xlabel("Tafel Slope Bin / mV decade$^-$$^1$")
    mt.ylabel("Overpotential Fit Width / V")
    ...
    x = array[:, 6]
    y = array[:, 9]
    z = array[:, 3]  # generate contours using Tafel slope bins.
    X, Y = np.meshgrid(x, y)
    Z = griddata((x, y), z, (X, Y), method="nearest")
    mt.contourf(X, Y, Z, levels)
    mt.colorbar(label="Tafel Slope / mV decade$^-$$^1$")
    # 2. Generate scatter plot:
    mt.plot(array[:, 6], array[:, 9], "k+")  # plot all fits.
    mt.plot(
        binned[:, 6], binned[:, 9], "co"
    )  # plot dV-optimized Tafel fits (spanning widest range of dVs).
    mt.plot(
        binnedR2s[:, 6], binnedR2s[:, 9], "ro"
    )  # plot dV+R2-optimimzed fits (highest R2s).
    mt.xlabel("Starting Fit Overpotential / V")
    mt.ylabel(
        "Ending Fit Overpotential / V"
    )  # ; mt.title('Voltage Ranges for Tafel Slope = m')
...
def binner(array, xcol_index, ycol_index, interval):
    array = array[
        array[:, xcol_index].argsort()
    ]  # sort array monotonically by Tafel slope (x) column.
    x_array = array[:, xcol_index]
    y_array = array[:, ycol_index]
    ...
    bin_index = np.digitize(
        x_array, bin_edges, right=True
    )  # np.digitize: returns index of the bin in bin_edges to which a value in x_array belongs.
    bin_value = []
    ...
    unique_dVs = np.zeros((len(first_index), 3))
    counter = 0
    unique_dVs[:, 0] = first_index
    while counter + 1 < len(first_index):
        unique_dVs[counter, 1] = len(
            np.unique(array[first_index[counter] : first_index[counter + 1], 2])
        )
        counter += 1
    ...
    mostdVs = indexer(max(unique_dVs[:, 1]), unique_dVs[:, 1])[
        0
    ]  # list of the starting indices of the widest-spanning Tafel bins.
    ...
    binned = np.zeros((int(l), 10))
    counter = 0
    for every in mostdVs:
        # find indices of unique dV values in each Tafel bin
        array_index = unique_dVs[int(every), 0]
        binsize = unique_dVs[int(every), 2]
        binned[counter : int(counter + binsize), :] = array[
            int(array_index) : int(array_index + binsize), :
        ]
        counter += int(binsize)

    # Find the fits with the maximal R2 in the Tafel bin.
    maxR2s = indexer(max(binned[:, 7]), binned[:, 7])[0]
    binnedR2s = np.zeros((len(maxR2s), 10))
    counter = 0
    for every in maxR2s:
    ...
    return (
        ...
        unique_dVs,
        maxR2s,
    )  # bin value of the optimum fit


def Plot_fit():
    print(
        "Use this for plotting the residue-minimized datasets generated through fitting (*minimized_residues.csv)."
    )
    print("Files to be analyzed must be in the current working directory.")
    filedir = os.getcwd() + "/*.csv"
    optimizations = np.sort(glob.glob(filedir))
    counter = 0
    ...
    f = int(
        input(
            "Which residue-optimized dataset do you want to inspect (enter file number)?: "
        )
    )
    bin_val = float(
        input("Enter a binning value for these data (1 mV/decade is typical): ")
    )
