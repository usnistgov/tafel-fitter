...
from common import approx_index, extract_filename, indexer
from Tafel_plot_user_v1p1 import plot_h

currentdir = os.getcwd()
direc = currentdir + "/Input_files/*.csv"
...
def Tfit_plot(
    ...
    os.makedirs(
        os.path.dirname(currentdir + "/Fits/" + saveto + "_n=" + str(n) + "/Binned/")
    )
    data = Vary_param_fit(
        filename, header, excelsheet, n, interval, delta_interval, rbound, delta_rbound
    )
...
def Vary_param_fit(
    ...
    fitcounts = 0
    recursions = 0
    figs_of_merit = np.zeros((len(V_int) * len(R2_int), 9))
    counter = 0
    for R2 in R2_int:
        for dV in V_int:
            ...
            if type(x[0]) == int:
                ...
                scantype = x[-1]
                fit_index = x[0][12]
                residual = x[0][0][fit_index]
                fit_start = x[0][1][0]
                slope = x[0][9][fit_index]
                io = x[0][2]
                R2_tafel = x[0][10][fit_index]
                R2_LSV = x[0][11][fit_index]
                if scantype == "cathodic":
                    figs_of_merit[counter, :] = (
                        R2,
                        dV,
                        slope,
                        io,
    ...
    print(
        "Total number of fit conditions tested (# of program recursions): "
        + str(recursions)
    )
    ...
    dataframe = pd.DataFrame(
        ...
        columns=[
            "R2 threshold",
            "Overpotential Fit Width / V",
            "Tafel slope / mV Decade-1",
            "Exchange Current / A",
            "Optimal Residual",
            "Starting Fit Potential / V",
            "Tafel R2",
            "LSV R2",
            "Ending Fit Potential",
        ],
    )
    dataframe.to_csv(
        currentdir
        ...
        + "/"
        + saveto
        + "_minimized_residues.csv"
    )
    # adjust the output array to match the array dimension for the Tafel_plot procedures
    figs_of_merit2 = np.zeros((len(figs_of_merit), 10))
    figs_of_merit2[:, 1:] = figs_of_merit
...
def Fit(filename, header, excelsheet, interval, rbound, n):
    ...
    if data[:, 0][-1] < 0:  # if data is cathodic:
        data = data[data[:, 0].argsort()][
            ::-1
        ]  # monotonically sort data in descending order by voltage (for cathode data).
        scantype = "cathodic"
    else:
        data = data[
            data[:, 0].argsort()
        ]  # monotonically sort data in ascending order by voltage (for anode data).
        scantype = "anodic"
    voltage = data[:, 0]
    lsv = data[:, 1]
    tafel = data[:, 2]
    ...
    if type(x) == int:
        ...
        fit_index = x[12]
        fitcount = x[-1]
        x1 = approx_index(x[1][0], voltage)[0][0]
        xn = approx_index(x[1][-1], voltage)[0][0]
        m = x[9][fit_index]
        b = x[2]
        chart = np.zeros((1, 4))
        chart[0, 0] = x[1][0]
        ...
        dataframe = pd.DataFrame(
            chart,
            columns=["Bound 1 (V)", "Bound 2 (V)", "Exchange Current", "Tafel Slope"],
        )
        dataframe.to_csv(
            currentdir
            + "/Fits/"
...
def fit_engine(voltage, ytafel, ylsv, interval, rbound, n):
    ...
    dfit = np.poly1d(np.polyfit(dx, dy, 15))
    p = np.poly1d(dfit)
    fitlist = []
    for every in dx:
    ...
    if bool(inflection):
        ...
        voltage_sub = voltage[:inflection]
        ytafel_sub = ytafel[:inflection]
        ylsv_sub = ylsv[:inflection]
    else:
        voltage_sub = voltage
        ytafel_sub = ytafel
        ylsv_sub = ylsv
    ...
    if voltage_sub[-1] > 0:  # Condition for anode data.
        while voltage_sub[counter] + interval < voltage_sub[-1]:
            ...
            test1 = scipy.stats.linregress(
                voltage_sub[counter : counter + index],
                ytafel_sub[counter : counter + index],
            )
            test2 = scipy.stats.linregress(
                voltage_sub[counter : counter + index],
                ylsv_sub[counter : counter + index],
            )
            # Test regions for linearity:
            dlogjdV = 1000 / (test1[0])
            J0 = 10 ** (test1[1])
            djdV = abs(test2[0])
            ...
            R2_LSV = test2[2] ** 2
            if (
                R2_Tafel >= rbound and R2_LSV >= rbound
            ):  # if R2 correlation is at least greater than some threshold for linearity (rbound):
                lst[0].append(J0)  # exchange currents
                lst[1].append(
                    [djdV, voltage_sub[counter]]
                )  # differentials and associated potentials
                lst[2].append(J0 * F / (n * R * T))
                lst[3].append(
                    [voltage_sub[counter], djdV - J0 * F / (n * R * T)]
                )  # differentials and associated potentials
                lst[4].append(R2_Tafel)
                lst[5].append(R2_LSV)
        ...
        while (
            voltage_sub[counter] - interval > voltage_sub[-1]
        ):  # Condition for cathode data
            index = approx_index(voltage_sub[counter] - interval, voltage_sub)[0][0]
            # stats.lingress output: [ slope, intercept, r, p_value, std_error ]
            test1 = scipy.stats.linregress(
                voltage_sub[counter : counter + index],
    ...
    for every in lst[1]:
        lst1_slopes.append(every[0])
        lst1_voltages.append(every[1])

    # extract potential range for fitting & Tafel parameters.
    residue = abs(np.array(lst1_slopes) - np.array(lst[2]))
    if len(residue) > 0:
        residue_index = approx_index(min(residue), residue)[0][
            0
        ]  # finding list index for lowest residue (J0*F/RT - dJ/dV)
        if voltage_sub[-1] > 0:
            fit_range = [
                lst1_voltages[residue_index],
                lst1_voltages[residue_index] + interval,
            ]
        else:
            fit_range = [
                lst1_voltages[residue_index],
...
def Run():
    ...
    if files[f][-4:] == ".csv":
        header = int(
            input(
                'Enter the last line of the file header (enter "0" if none present): '
            )
        )
        ...
        dx, dy = derivative(voltage, lsv)
        # perform least-squares fit of derivative LSV, find function's maximum:
        dfit = np.poly1d(np.polyfit(dx, dy, fit_order))
        p = np.poly1d(dfit)
        fitlist = []
        for every in dx:
            fitlist.append(p(every))
        ...
        plt.subplot(2, 1, 1)
        plt.plot(dx, dy, "ko")
        plt.plot(dx, fitlist, "c", linewidth=2.0)
        ...
        if accept == "Y" or "y":
            interval = eval(
                input(
                    "Enter the range of overpotential window sizes (dV) to check, in volts (i.e., [0.01, 0.05] ): "
                )
            )
            delta_interval = float(
                input("Enter the dV increment to use, in volts (i.e., 0.001): ")
            )
            rbound = eval(
                input(
                    "Enter the range of R-squared values over which to conduct the fits (i.e., [0.900, 1.000] ): "
                )
            )
            delta_rbound = eval(
                input(
                    "Enter the increment for the R-squared value range (i.e., 0.001): "
                )
            )
            binning = float(
                input(
                    "Enter the binning size to use for dV-span optimization, in millivolts (1 mV default): "
                )
            )
            Tfit_plot(
                files[f],
